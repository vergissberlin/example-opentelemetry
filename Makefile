...
define SERVICE_STATUS
	...
	@for item in ${2}; do \
		if [ $$(docker compose -f ${3}/$${item}/compose.yml ps --services --status running | wc -l) -gt 0 ]; then \
			docker compose -f ${3}/$${item}/compose.yml ps --format "table {{printf \"%-12s\" .ID}}  {{printf \"%-34s\" .Name}}  {{printf \"%-10s\" .State}}  {{if gt (len .Publishers) 0}}{{with index .Publishers 0}}{{printf \"http://0.0.0.0:%d\" .PublishedPort}}{{end}}{{else}}No published ports{{end}}"; \
		fi; \
		if [ $$(docker compose -f ${3}/$${item}/compose.yml ps --services  --status exited --status paused --status restarting --status removing --status dead --status exited | wc -l) -gt 0 ]; then \
			...
			docker compose -f ${3}/$${item}/compose.yml ps --format "table {{printf \"%-12s\" .ID}}  {{printf \"%-34s\" .Name}}  {{printf \"%-10s\" .State}} " --status exited --status paused --status restarting --status removing --status dead --status exited]; \
