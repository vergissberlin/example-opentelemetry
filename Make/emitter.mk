...
emitter-%-up: .logo ## Starts the emitter in the specified language
	...
	@echo -e "${PBEGIN}docker compose -f examples/emitter/$*/compose.yml up -d${PEND}\n"
	@docker compose -f examples/emitter/$*/compose.yml up -d

emitter-%-restart: .logo ## Restarts the emitter in the specified language
	@echo -e "${H2BEGIN}Restart emitter with ${*}${H2END}"
	@echo -e "${PBEGIN}docker compose -f examples/emitter/$*/compose.yml down -v${PEND}\n"
	@docker compose -f examples/emitter/$*/compose.yml down -v
	@echo -e "${PBEGIN}docker compose -f examples/emitter/$*/compose.yml up -d${PEND}\n"
	@docker compose -f examples/emitter/$*/compose.yml up -d
